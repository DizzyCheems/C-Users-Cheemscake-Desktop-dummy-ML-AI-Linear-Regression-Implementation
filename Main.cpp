...
double alpha = 0.05;             //intializing error rate
int n;							 // Define number of Generations	
char choice;						
  
/*Training Phase*/
cout<< "Enter number of loops (5 values * epoch = loop):"<<endl;
cin>> n;

for (int i = 0; i < n; i++) {     	// since there are 5 values and we want 4 epochs so run for loop for 20 times
...
system ("pause");
output ();
cout<<"Type"<< "(Y)" <<"to run another test, "<< "(N)" <<" to exit the program"<<endl;
cin>>choice;
output ();
